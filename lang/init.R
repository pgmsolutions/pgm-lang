...
rpgm.setLang <- function(lang = NULL)
{
    ...
    if(is.null(lang))
        ...
        .lang <<- list(lang = lang)
    if(file.exists(rpgm.pgmFilePath(paste0("lang/", .lang$lang, ".R"))))
        source(rpgm.pgmFilePath(paste0("lang/", .lang$lang, ".R")), encoding= "utf8"); return(TRUE)
    if(file.exists(rpgm.pgmFilePath(paste0("lang/", Default, ".R"))))
        source(rpgm.pgmFilePath(paste0("lang/", Default, ".R")), encoding= "utf8"); return(TRUE)
    return(FALSE)
}
...
gui.updateLang <- function(gui)
{
    if(exists("lang"))
        rpgm.setLang(lang)
    if(is.null(.lang[[gui]]))
        stop(sprintf(.lang$init.R$gui.updateLang$error, gui))
